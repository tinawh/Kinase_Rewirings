...
tcga.modules <- readRDS("/.mounts/labs/reimandlab/private/users/thuang/data_2/03-05-18/modules_added_nonlinkers_permut_10.rds" )
...
mimp.modules <- readRDS("/.mounts/labs/reimandlab/private/users/thuang/data_2/03-05-18/mimp_modules_permut_10.rds")

census.genes <- fread("/.mounts/labs/reimandlab/private/users/thuang/data_2/read-only/Census_allMon Jan 22 18_25_23 2018.tsv")

# file names
scurves.file <- "/.mounts/labs/reimandlab/private/users/thuang/data_2/03-05-18/pcawg_scurves_permut_10.pdf"
tcga.pcawg.scurves.file <- "/.mounts/labs/reimandlab/private/users/thuang/data_2/03-05-18/tcga_pacwg_scurves_permut_10.pdf"
mutation.matrix.file <- "/.mounts/labs/reimandlab/private/users/thuang/data_2/03-05-18/pcawg_mutation_matrices_permut_10.pdf"
tcga.mutation.matrix.file <- "/.mounts/labs/reimandlab/private/users/thuang/data_2/03-05-18/tcga_pcawg_mutation_matrices_permut_10.pdf"
small.modules.filtered.file <- "/.mounts/labs/reimandlab/private/users/thuang/data_2/03-05-18/pcawg_modules_permut_10.rds"
	...
	for (cancer in names(filtered.muts)) { 
		...
		for (df in names(filtered.muts.cancer)) {
			...
			if (nchar(df) < 50) {
				...
				if (length(splits) <= 20) {
					if (length(splits <= 10)) {
						title <- paste("Module: ", paste(splits, collapse = ";"), sep = "")
					} else {
	...
	for (cancer in names(full.muts)) { 
		...
		for (df in names(full.muts.cancer)) {
			...
			if (length(setdiff(b, a)) != 0) {
				...
				joined <- mutate(tr, mimp = FALSE)
				lst[[paste(cancer.type, df)]] <- joined
		}
	}
	return(lst)
}
