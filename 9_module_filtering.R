...
# file names
nosubset.results.file <- "/.mounts/labs/reimandlab/private/users/thuang/data_2/01-08-18/nosubset_hypermodules_results.rds"
...
ReadHypermodulesResults <- function(hypermodules.dir, hypermodules.prefix) {
	...
	hypermodules.results <- list() 

	lst <- lapply(file.paths, function(fil) {fread(fil, skip = 3)})
...
SplitClinicalResults <- function(clins) {
	splitted <- split(clins, clins$Study_Name)
	return(splitted)
}
